    ...
    class Methods
    {
        static double CalculateTriangleArea (double sideA, double sideB, double sideC)
        {
            if (sideA <= 0 || sideB <= 0 || sideC <= 0)
            {
                ...
                //return -1;
                throw new ArgumentException ("The sides should be positive numbers bigger than zero !");
            }
        ...
        static string DigitToEnglishName (int digit)
        {
            switch (digit)
            {
                ...
                default:
                    {
                        throw new ArgumentOutOfRangeException("Invalid number ! The number is out of range !");
                    }
            }
        }

        static int FindBiggestElement (params int[] elements)
        {
            if (elements == null || elements.Length == 0)
            {
                //return -1;
                throw new ArgumentNullException ("The array cannot be null, or with length equal to zero !");
            }
        ...
        static void PrintNumberInFormat (object numberObj, string signForFormat)
        {
            //if (format == "f")
            //{
        ...
        private static string GetFormatForPrint (string format)
        {
            switch (format)
            {
                ...
                default:
                    {
                        throw new ArgumentException ("Invalid format ! Not supported format for print !");
                    }
            }
        }
        ...
        static void Main ()
        {
            ...
            Console.WriteLine (DigitToEnglishName (5));
