...
class E13GetAllListedWordsAndSort
{
    static void Main ()
    {
        ...
        // in the file result.txt and the words should be sorted by the number of their occurrences in 
        // descending order. Handle all possible exceptions in your methods.

        try
        {
            string[] listedWords = File.ReadAllLines ("words.txt", Encoding.GetEncoding ("windows-1251"));
            ...
            using (StreamReader reader = new StreamReader ("test.txt", Encoding.GetEncoding ("windows-1251")))
            {
                ...
                while (line != null)
                {
                   ...
                   {
                        counter[index] += ((Regex.Matches (line, @"\b" + listedWords[index] + @"\b").Count));
                   }
