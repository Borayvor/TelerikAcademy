...
class E13MergeSort
{
    ...
    static int[] Splits (int[] array)
    {
        ...
        // Слагаме в първия масив(left), половината от елементите на подадения масив (array).
        for (int index = 0; index < middle; index++)
        {
            leftArray[index] = array[index];
        }
        // Слагаме във втория масив(right), другата половината от елементи на подадения масив (array).
        for (int index = middle, j = 0; index < array.Length; index++, j++)
        {
            rightArray[j] = array[index];
        }

        leftArray = Splits (leftArray); //Вика се рекурсия на лявата половина, докато нейната дължина не стане равна на 1.
        rightArray = Splits (rightArray); // След като свършим изцяло с лявата половина на първоначално подадения масив,
        // прави се същото и с дясната половина, докато не се изчерпят всички нейни стойности

        return Merge (leftArray, rightArray); //Когато в двата масива остане само по 1 елемент, викаме метода
    ...
    static int[] Merge (int[] left, int[] right)
    {
        ...
        for (int index = 0; index < array.Length; index++)
        {
            // Ако левият елемент е по-малък, той се записва в масива (array) и се увеличава левия брояч (leftIncrease) с 1
